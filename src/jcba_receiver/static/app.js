const elements = {
  ...
  region: document.querySelector('#station-region'), count: document.querySelector('#station-count'), connection: document.querySelector('#connection-label'), play: document.querySelector('#play-button'),
...
function status(label, notice = '', error = false) { elements.state.textContent = label; elements.connection.textContent = label; elements.notice.textContent = notice; elements.notice.classList.toggle('error', error); }
function resetPlaybackVisuals() { elements.playIcon.textContent = '▶'; elements.play.setAttribute('aria-label', '再生'); elements.wave.classList.remove('playing'); }
...
function stop() { state.playbackId += 1; elements.audio.pause(); elements.audio.removeAttribute('src'); elements.audio.load(); resetPlaybackVisuals(); status('STOPPED', '再生を停止しました。'); }
elements.audio.addEventListener('playing', () => { elements.playIcon.textContent = '■'; elements.play.setAttribute('aria-label', '停止'); elements.wave.classList.add('playing'); status('PLAYING', 'ライブ音声を受信中です。'); }); elements.audio.addEventListener('waiting', () => { if (!elements.audio.paused) status('BUFFERING', 'バッファリングしています。'); }); elements.audio.addEventListener('error', () => { if (!elements.audio.error) return; state.playbackId += 1; elements.audio.pause(); elements.audio.removeAttribute('src'); elements.audio.load(); resetPlaybackVisuals(); status('UNAVAILABLE', '現在この局の配信を受信できません。時間をおいて再試行してください。', true); }); elements.play.addEventListener('click', play); elements.favorite.addEventListener('click', () => { if (!state.selected) return; const id = state.selected.id; state.favorites.has(id) ? state.favorites.delete(id) : state.favorites.add(id); saveFavorites(); selectStation(id); }); elements.favoritesFilter.addEventListener('click', () => { state.favoritesOnly = !state.favoritesOnly; render(); }); elements.search.addEventListener('input', event => { state.query = event.target.value; render(); }); elements.volume.addEventListener('input', event => { elements.audio.volume = event.target.value / 100; }); elements.mute.addEventListener('click', () => { elements.audio.muted = !elements.audio.muted; elements.mute.textContent = elements.audio.muted ? '◌' : '◖'; elements.mute.setAttribute('aria-pressed', String(elements.audio.muted)); elements.mute.setAttribute('aria-label', elements.audio.muted ? 'ミュート解除' : 'ミュート'); });
