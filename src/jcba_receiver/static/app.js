...
const state = { stations: [], selected: null, region: '', query: '', favoritesOnly: false, favorites: new Set(JSON.parse(localStorage.getItem('jcba-favorites') || '[]')) };
...
async function selectStation(id) { const station = state.stations.find(s => s.id === id); if (!station) return; if (!elements.audio.paused) stop(); state.selected = station; elements.name.textContent = station.name; elements.prefecture.textContent = `${station.prefecture} · ${station.region}`; elements.region.textContent = station.id.toUpperCase(); elements.favorite.setAttribute('aria-pressed', String(state.favorites.has(id))); elements.favorite.textContent = state.favorites.has(id) ? '★' : '☆'; elements.programTitle.textContent = '番組情報を取得中'; elements.programPerformer.textContent = 'この局が情報を提供している場合に表示されます。'; elements.programDetail.textContent = ''; status('READY', '再生ボタンでライブ受信を開始します。'); render(); try { const response = await fetch(`/api/programs/${id}`); const { program } = await response.json(); elements.programTitle.textContent = program?.title || '番組情報はありません'; elements.programPerformer.textContent = program?.performer || '現在の番組情報は提供されていません。'; elements.programDetail.textContent = program?.detail || ''; } catch { elements.programTitle.textContent = '番組情報を取得できません'; } }
async function play() { if (!state.selected) { status('STANDBY', 'まず放送局を選択してください。', true); return; } if (!elements.audio.paused) return stop(); status('CONNECTING', 'ライブ配信へ接続しています。'); elements.audio.src = `/api/stream/${state.selected.id}?t=${Date.now()}`; elements.audio.load(); try { await elements.audio.play(); } catch { status('ERROR', '再生を開始できませんでした。もう一度お試しください。', true); } }
function stop() { elements.audio.pause(); elements.audio.removeAttribute('src'); elements.audio.load(); elements.playIcon.textContent = '▶'; elements.play.setAttribute('aria-label', '再生'); elements.wave.classList.remove('playing'); status('STOPPED', '再生を停止しました。'); }
elements.audio.addEventListener('playing', () => { elements.playIcon.textContent = '■'; elements.play.setAttribute('aria-label', '停止'); elements.wave.classList.add('playing'); status('PLAYING', 'Ogg/Opus ライブを受信中です。'); }); elements.audio.addEventListener('waiting', () => { if (!elements.audio.paused) status('BUFFERING', 'バッファリングしています。'); }); elements.audio.addEventListener('error', () => { if (elements.audio.error) status('UNAVAILABLE', '現在この局の配信を受信できません。時間をおいて再試行してください。', true); }); elements.play.addEventListener('click', play); elements.favorite.addEventListener('click', () => { if (!state.selected) return; const id = state.selected.id; state.favorites.has(id) ? state.favorites.delete(id) : state.favorites.add(id); saveFavorites(); selectStation(id); }); elements.favoritesFilter.addEventListener('click', () => { state.favoritesOnly = !state.favoritesOnly; render(); }); elements.search.addEventListener('input', event => { state.query = event.target.value; render(); }); elements.volume.addEventListener('input', event => { elements.audio.volume = event.target.value / 100; }); elements.mute.addEventListener('click', () => { elements.audio.muted = !elements.audio.muted; elements.mute.textContent = elements.audio.muted ? '◌' : '◖'; });
(async function init() { try { const response = await fetch('/api/stations'); const data = await response.json(); state.stations = data.stations; elements.count.textContent = `${data.count} STATIONS`; render(); const initial = state.stations.find(s => s.id === 'fmnanami') || state.stations[0]; if (initial) selectStation(initial.id); const refreshed = await fetch('/api/stations?refresh=true'); const refreshedData = await refreshed.json(); if (refreshedData.count !== data.count) { state.stations = refreshedData.stations; elements.count.textContent = `${refreshedData.count} STATIONS`; render(); } } catch { status('ERROR', '局一覧を取得できませんでした。ページを再読み込みしてください。', true); } })();
